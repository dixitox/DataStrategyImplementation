...
function BlockOptionsDropdownList({ editor, blockType, toolbarRef, setShowBlockOptionsDropDown }) {
    ...
    useEffect(() => {
        ...
        if (toolbar !== null && dropDown !== null) {
            ...
            dropDown.style.top = `${top + window.pageYOffset + 40}px`;
...
interface ToolbarPluginProps {
    uploadImage?: (base64Image: string | ArrayBuffer | null) => Promise<string>;
}
...
export default function ToolbarPlugin(props: ToolbarPluginProps) {
    ...
    const insertImage = useCallback(() => {
        // Pick user image or ask for url
        if (props.uploadImage) document.getElementById("toolbar-input-image")!.click();
        else {
            const srcUrl = prompt("Enter the URL of the image:", "");
            if (srcUrl)
    ...
    const pickImage = useCallback(async (ev: ChangeEvent<HTMLInputElement>): Promise<void> => {
        const file = ev.target.files![0];
        const base64 = await convertBase64Async(file);
        const srcUrl = await props.uploadImage!(base64);
        if (srcUrl)
            editor.dispatchCommand(INSERT_IMAGE_COMMAND, {
                altText: "Image",
                src: srcUrl,
        ...
        <div className="toolbar" ref={toolbarRef}>
            ...
            {blockType === "code" ? (
                ...
                <>
                    ...
                    <button onClick={insertImage} className="toolbar-item spaced" aria-label="Insert Image">

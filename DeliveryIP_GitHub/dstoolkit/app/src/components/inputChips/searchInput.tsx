...
const SearchInput: FC<Props> = (props) => {
    ...
    const { boxTopPosition, checkHeight, addEventListener, removeEventListener, selectedIndex, onKeyPress } =
    ...
    return (
        <div className="input_container" ref={inputContainerRef} onKeyDown={onKeyDownItem}>
            <div className="search_container">
                ...
                >
                    {loadingSuggestions ? (
                        <div>
                            <div className="nothing_found_container">{props.loadingSpinner || <div>loading</div>}</div>
                        </div>
                    ) : (
                        <div>
                            {nothingFound ? (
                                props.emptyMessage ? (
                                    <div className="nothing_found_container">{props.emptyMessage}</div>
                                ) : null
                            ) : (
                                <span>
                                    {hitList.map((item, index) =>
                                        props.renderListItem(selectedIndex === index, item, selectClickedElement)
                                    )}
                                </span>
                            )}
                        </div>

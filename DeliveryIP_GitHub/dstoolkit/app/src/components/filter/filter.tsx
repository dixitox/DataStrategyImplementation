...
export function Filter({ className, facetsCounts, onFilterChanged }: FilterProps) {
    ...
    async function loadAllFacetsAsync() {
        const facetResult: Facet[] = await httpClient.post(`${window.ENV.API_URL}/assets/facets`);
    ...
    return (
        <div className={`${className || ""} flex flex-col`}>
            ...
            {!window.ENV?.RUN_FLAGS.includes(RunFlags.NoFiltersIndustries) && (
                <>
            ...
            {!window.ENV?.RUN_FLAGS.includes(RunFlags.NoFiltersMachineLearning) && (
                <>
                    <span className="mb-2 text-base font-semibold">{t("components.filter.machine-learning")}</span>
                    {/* <Checkbox
                className="-ml-2"
                checked={checkedTags.length === allTags.length ? true : checkedTags.length > 0 ? "mixed" : false}
                onChange={(_ev, data) => (data.checked ? setCheckedTags([...allTags]) : setCheckedTags([]))}
                label={`${t("common.all")} (${
                    facetsCounts?.find((f) => f.name === FacetType.Tags)?.totalResults || 0
                })`}
            /> */}
                    {allTags.map((item, idx) => {
                        return (
                            <Checkbox
                                ...
                                onChange={(_ev, data) =>
                                    data.checked
                                        ? setCheckedTags((o) => [...o, item])
                                        : setCheckedTags((o) => o.filter((o) => o !== item))
                                }
                                label={`${item} (${countByTag[item] || 0})`}
                            />
                        );
                    })}
                    <span className="my-5 border-b border-b-neutral-300" />
            ...
            {!window.ENV?.RUN_FLAGS.includes(RunFlags.NoFiltersAssetType) && (
                <>
                    <span className="mb-2 text-base font-semibold">{t("components.filter.asset-type")}</span>
                    {allTypes.map((item, idx) => {
                        return (
                            <Checkbox
                                ...
                                onChange={(_ev, data) =>
                                    data.checked
                                        ? setCheckedTypes((o) => [...o, item])
                                        : setCheckedTypes((o) => o.filter((o) => o !== item))
                                }
                                label={`${item} (${countByType[item] || 0})`}
                            />
                        );
                    })}
                    <span className="my-5 border-b border-b-neutral-300" />

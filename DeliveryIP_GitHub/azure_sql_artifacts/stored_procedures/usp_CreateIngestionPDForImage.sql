CREATE OR ALTER PROCEDURE [dbo].[usp_CreateIngestionPDForImage]
@body nvarchar(max)
AS
BEGIN
...
VALUES (
--SourceObjectSettings
'{ "fileName": "' + JSON_VALUE(@body, '$."PDF File Name"') + '",
...
,''
--CopySourceSettings
,'{ "model": "' + JSON_VALUE(@body, '$."Select Model"') + '" }'
--SinkObjectSettings
,'{ "fileName": "' + JSON_VALUE(@body, '$."PDF File Name"') + '", "folderPath": "' + JSON_VALUE(@body, '$.dynamicSinkPath') + '",  "container": "' + JSON_VALUE(@body, '$."Sink Container"') + '" }'
--SinkConnectionSettingsName
,''
--CopySinkSettings
,''

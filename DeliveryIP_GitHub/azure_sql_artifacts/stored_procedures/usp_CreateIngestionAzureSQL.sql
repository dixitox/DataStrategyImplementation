...
BEGIN
 BEGIN TRY
    ...
    DECLARE @AzureSQLFileName NVARCHAR(MAX)
    DECLARE @AzureSQLFolderPath NVARCHAR(MAX)
...
SELECT @dataSetName  = value FROM @values WHERE [index] = 3;
    SET @AzureSQLFileName = 'OperationsRiskManagement_AzureSQL_' + @subjectArea + '_' + JSON_VALUE(@body, '$."Table Schema"') + '_' + JSON_VALUE(@body, '$."Table Name"') + '_YYYYMMDDHHMMSS.parquet'
PRINT '@AzureSQLFileName: ' + @AzureSQLFileName;    
    SET @AzureSQLFolderPath = 'OperationsRiskManagement/AzureSQL/' + @subjectArea + '/' + JSON_VALUE(@body, '$."Table Schema"') + '/' + JSON_VALUE(@body, '$."Table Name"') + '/'
    SET @TriggerName = 'TR_OperationsRiskManagement_AzureSQL_' + @subjectArea + JSON_VALUE(@body, '$."Table Schema"') + '_' + JSON_VALUE(@body, '$."Table Name"')
	SET @query = 'SELECT * FROM ' + JSON_VALUE(@body, '$."Table Schema"') + '.' + JSON_VALUE(@body, '$."Table Name"') + ' WHERE '+ JSON_VALUE(@body, '$."Watermark Column"') + ' > CAST(''WATERMARKVALUE'' AS datetime)';
    -- RECORD 1 
    ...
    VALUES ( 
        ...
        --SinkObjectSettings
        ,'{ "fileName": "' + @AzureSQLFileName + '","folderPath": "' + @AzureSQLFolderPath + '","container": "' + JSON_VALUE(@body, '$."Source Container"') + '" }'
        --SinkConnectionSettingsName
        ,''
        --CopySinkSettings
        ,''
    ...
    VALUES (
        --SourceObjectSettings
        '{  "fileName": "%.parquet","folderPath": "' + @AzureSQLFolderPath + 'full/%' + '","container": "' + JSON_VALUE(@body, '$."Source Container"') + '" }'  -- where is '/full' and '/incremental' coming from? 
        --SourceConnectionSettingsName
        ,''
        --CopySourceSettings
        ,'{ "fileType": "parquet","compression":"" }'
        --SinkObjectSettings
        ,'{ "fileName": null, "folderPath": "' + @AzureSQLFolderPath + '", "container": "' + JSON_VALUE(@body, '$."Sink Container"') + '" }'
        --SinkConnectionSettingsName
        ,''
        --CopySinkSettings
        ,''
 ...
 VALUES (
        --SourceObjectSettings
        '{ "fileName": "%.parquet","folderPath": "' + @AzureSQLFolderPath + 'incremental/%' + '","container": "' + JSON_VALUE(@body, '$."Source Container"') + '" }'
        --SourceConnectionSettingsName
        ,''
        --CopySourceSettings
        ,'{ "fileType": "parquet","compression":"" }'
...
/*   This require changes in the Pattern Table and the PowerApp
 ...
 VALUES (
        --SourceObjectSettings
		'{ "fileName": "%.parquet","folderPath": "' + @AzureSQLFolderPath + '%", "container": "' + JSON_VALUE(@body, '$."Sink Container"') + '" }'
        --SourceConnectionSettingsName
        ,''
        --CopySourceSettings
        ,'{ "primary_key_cols": "' + JSON_VALUE(@body, '$."Primary Key Columns"') + '","partition_cols": "","date_partition_column":"","file_type":"" }'
        --SinkObjectSettings
        ,'{ "fileName": null,"folderPath": "' + @AzureSQLFolderPath + '","container": "staging" }'
        --SinkConnectionSettingsName
        ,''
        --CopySinkSettings
        ,''
